(() => {
    ...
    getElement(".modal__body", modal || document);
    location.origin;
    ...
    function headerFixed() {
        if (header) {
            ...
            window.addEventListener("scroll", (() => {
                ...
                try {
                    if (!header.classList.contains("active")) if (scrollY >= header.clientHeight / 2.5) header.classList.add("fixed"); else header.classList.remove("fixed");
    ...
    function productionSwiper() {
        initSwiper('[data-swiper="production"]', ".production-next", ".production-prev");
    }
    function response() {
